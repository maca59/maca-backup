...
<header class="maca-bp-header">
	<div class="maca-bp-header__brand">
		<img
			...
			alt="<?php echo esc_attr__( 'maca BackUp', 'maca-backup-pro' ); ?>"
			width="76"
			height="76"
		/>
		<div>
			<h1 class="maca-bp-header__title"><?php esc_html_e( 'maca BackUp', 'maca-backup-pro' ); ?></h1>
			<p class="maca-bp-header__tagline"><?php esc_html_e( 'Backup, restore & Smart Restore for WordPress.', 'maca-backup-pro' ); ?></p>
		</div>
	</div>
	<div class="maca-bp-header__meta">
		<?php if ( $failed_banner ) : ?>
			<span class="maca-bp-badge maca-bp-badge--fail" title="<?php echo esc_attr( (string) ( $failed_banner->error_message ?? '' ) ); ?>">
				<?php esc_html_e( 'Backup failed', 'maca-backup-pro' ); ?>
			</span>
		<?php endif; ?>
...
<?php if ( $failed_banner ) : ?>
	...
	<div class="maca-bp-fail-banner" role="alert">
		<div class="maca-bp-fail-banner__text">
			<strong><?php esc_html_e( 'Last backup failed', 'maca-backup-pro' ); ?></strong>
			<span>
				<?php
				printf(
					/* translators: 1: backup type, 2: datetime */
					esc_html__( '%1$s · %2$s', 'maca-backup-pro' ),
		...
		<div class="maca-bp-fail-banner__actions">
			<a class="button button-small" href="<?php echo esc_url( Maca_Backup_Pro_Admin::tab_url( 'logs' ) ); ?>"><?php esc_html_e( 'Logs', 'maca-backup-pro' ); ?></a>
			<a class="button button-small" href="<?php echo esc_url( Maca_Backup_Pro_Admin::tab_url( 'backups' ) ); ?>"><?php esc_html_e( 'Backups', 'maca-backup-pro' ); ?></a>
			<a class="maca-bp-fail-banner__dismiss" href="<?php echo esc_url( $fail_dismiss ); ?>"><?php esc_html_e( 'Dismiss', 'maca-backup-pro' ); ?></a>
		</div>
	</div>
<?php endif; ?>

<?php if ( ! empty( $tabs ) ) : ?>
	<nav class="maca-bp-tabs<?php echo $failed_banner ? ' maca-bp-tabs--after-fail' : ''; ?>" aria-label="<?php esc_attr_e( 'maca BackUp sections', 'maca-backup-pro' ); ?>">
